    ...
    <div class="grid grid-cols-2 gap-4 max-[785px]:grid-cols-1">
        ...
        <div class="p-5 mb-4 border border-gray-100 rounded-lg bg-gray-50">
            ...
            @forelse($reservas as $reserva)
                <time
                    class="text-lg font-semibold text-gray-900">{{ Carbon\Carbon::parse($reserva->created_at)->diffForHumans(['parts' => 1]) }}</time>
                <ol class="mt-3 divide-y divider-gray-200">
                    <li>
                        <a href="{{ route('admin.reserva.detalles', $reserva->id) }}">
                            <div class="text-base font-normal">{{ $reserva->user->name }} {{ $reserva->user->apellidos }}
                                ha
                                reservado la actividad <span class="text-black underline">{{ ucfirst($reserva->actividad->titulo) }}</span>.
                            </div>
                        </a>
                    </li>
                </ol>
