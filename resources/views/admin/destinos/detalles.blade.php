            ...
            <tbody>
                <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                    ...
                    <td class="px-6 py-4 flex justify-between items-center">
                        <a href=" {{ route('admin.editarDestino', $destino->id) }} "
                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Editar</a>
                        <form action=" {{ route('admin.borrarDestino', $destino->id) }} " method="POST" class="inline">
                            @csrf
                            <input type="hidden" name="id" value="{{ $destino->id }}">
                            <button type="submit" onclick="cambiar(event, {{ $destino->id }})"
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-6 text-center">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">¿Estás seguro de querrer borrar a
                        este destino?</h3>
                    <form action="{{ route('admin.borrarDestino', $destino->id) }}" method="post">
                        @method('DELETE')
                        @csrf
